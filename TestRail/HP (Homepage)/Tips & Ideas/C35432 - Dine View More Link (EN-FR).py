...
class YPTipsAndIdeas:
    ...
    def tips_ideas_dine_view_more(self, link):
        """
        ...
        if tips_ideas_container:
            ...
            if link == My.yp_web_link:
                if driver.current_url[0:32] == link + "/dine/":
                    YPTipsAndIdeas.is_success = True
                    return
                else:
                    return
            else:
                if driver.current_url[0:33] == link + "/resto/":
                    YPTipsAndIdeas.is_success = True
                    return
                else:
                    return

    def testing_tips_ideas_dine_view_more(self, link):
        """
        >> This function will execute the test case, and takes a link as a parameter
        """
        My.search_merchant_page(driver, link)
        test.tips_ideas_dine_view_more(link)
...
test.testing_tips_ideas_dine_view_more(My.yp_web_link)
print('----------')
test.testing_tips_ideas_dine_view_more(My.pj_web_link)
driver.quit()
sys.exit()
